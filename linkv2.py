...
>=>      >=>    >=>    >=>      >=>     >===>          >===>           >=>     
          ############## ReverseIPLookup ##############                                                                     		 
	    Telegram Channels => https://t.me/FoxCyberSecurity
	...
	try:
		...
		if 'response_code' in grab:
			...
			for rr in res:
				...
				open('Resultswebsite.txt', 'a').write('http://'+resk+'\n')
			else:
		...
		for page in range(int(Shinn), int(Codde)):
			...
			if 'Recent IP reviews' in Shin:
				...
				for xxx in Shine:
					...
					open('x7root_IPs.txt', 'a').write(Repshin+'\n')
				else:
					print(Fore.RED + 'Coded By x7root' + Fore.WHITE)
	except:
		pass
